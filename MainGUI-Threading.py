...
import threading								#Multithreading. Way faster than multiprocessing.
...
def reqProc (q, result):								#Requests urls and processes it. Asyncronous Multiprocessing. The bodged way. Don't need BeautifulSoup when you can just request for the json, much less crap to sift out.
	while not q.empty():
		work = q.get()
		response = requests.get(work[1])
		response = response.text
		result[work[0]] = response.replace('{"counts": {"posts"', '').replace('}}', '').replace(':', '')
		q.task_done()
	return True

def progbar (querylist, q, numthreads):
	count = 0
...
def main():
	...
	numthreads = min(50, total)							#Number of threads allowed to use at any time, otherwise may overwhelm. Does introduce a slight speed bump for larger sets than 100.
	list2 = progbar(query1, q, numthreads)
	...
	list3 = progbar(query2, q, numthreads)
